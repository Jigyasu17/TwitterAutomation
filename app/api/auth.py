...
def verify_admin_auth(authorization: str = Header(None)):
    """Restricts manual mutating endpoints (collect/process/approve/reject/
    research/draft actions) to callers holding ADMIN_SECRET. Skipped in
    development so local iteration needs no token."""
    if settings.ENV == "development":
        return
